...
MessageTable::MessageTable(std::string db_dir_path) {
    ...
    if (!std::filesystem::exists(this->file_path)) {
        ...
        if (file) {
            ...
            file << "snowflake|sender_uid|channel_uid|created_at|modified_at|read_by|text"
                 << std::endl;
        } else {
                ...
                if (!isHeader) {
                    ...
                    if (tokens.size() >= 7) {
                        try {
                            ...
                            auto message =
                                std::make_shared<Message>(sender_uid, channel_uid, text, snowflake,
                                                          created_at, modified_at, read_by);
...
std::variant<Message::SharedPtr, std::string> MessageTable::add_message(
    UUID sender_uid,
    UUID channel_uid,
    std::string content,
    ...
    if (snowflake.has_value() && this->data.find(snowflake.value()) != this->data.end()) {
        return "Message with the same snowflake already exists : " +
               std::to_string(snowflake.value());
    }

    auto message =
        snowflake.has_value()
            ? std::make_shared<Message>(sender_uid, channel_uid, content, snowflake.value(),
                                        created_at.value_or(0), modified_at.value_or(0),
                                        read_by.value_or(std::vector<UUID>{}))
            : std::make_shared<Message>(sender_uid, channel_uid, content);
    ...
    file << message->get_snowflake() << "|" << message->get_sender_id().to_string() << "|"
         << message->get_channel_id().to_string() << "|" << message->get_created_at() << "|"
         << message->get_modified_at() << "|" << read_by_str << "|" << message->get_text() << "\n";
