...
ConnectionWindow::ConnectionWindow(QWidget* parent) : QWidget(parent) {
    ...
    // Hostname field
    QRegularExpressionValidator* host_validator =
        new QRegularExpressionValidator(QRegularExpression("^\\S+$"), this);
    ValidatedTextInput* host = new ValidatedTextInput(
        inputGroup, "hostInput", "Hostname", host_validator, QLineEdit::EchoMode::Normal,
        validate_text(host_validator), std::bind(&ConnectionWindow::on_submit, this));

    // Port field
    QIntValidator* port_validator = new QIntValidator(0, 65535, this);
    ValidatedTextInput* port = new ValidatedTextInput(
        inputGroup, "portInput", "Port", port_validator, QLineEdit::EchoMode::Normal,
        validate_text(port_validator), std::bind(&ConnectionWindow::on_submit, this));
...
std::function<bool(const QString&, QWidget*)> ConnectionWindow::validate_text(
