...
void User::deserialize(const std::vector<uint8_t> &buf)
{
    ...
    this->display_name = std::string(buf.begin() + offset, buf.begin() + offset + display_name_length);
    offset += display_name_length;

    this->profile_pic = std::string(buf.begin() + offset, buf.begin() + offset + profile_pic_length);
